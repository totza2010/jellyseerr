...
import type {
  IgnoreItem,
  IgnoreResultsResponse,
} from '@server/interfaces/api/ignoreInterfaces';
...
const messages = defineMessages('components.Ignore', {
  ignoresettings: 'Ignore Settings',
  ignoreSettingsDescription: 'Manage ignored media.',
  mediaName: 'Name',
  mediaType: 'Type',
  mediaTmdbId: 'tmdb Id',
  ignoredate: 'date',
  ignoredby: '{date} by {user}',
  ignoreNotFoundError: '<strong>{title}</strong> is not ignored.',
});
...
const Ignore = () => {
  ...
  const {
    ...
    error,
    mutate: revalidate,
  } = useSWR<IgnoreResultsResponse>(
    `/api/v1/ignore/?take=${currentPageSize}
  ...
  return (
    <>
      <PageTitle title={[intl.formatMessage(globalMessages.ignore)]} />
      <Header>{intl.formatMessage(globalMessages.ignore)}</Header>
      ...
      {!data ? (
        ...
        data.results.map((item: IgnoreItem) => {
          return (
            <div className="py-2" key={`request-list-${item.tmdbId}`}>
              <IgnoredItem item={item} revalidateList={revalidate} />
            </div>
          );
        })
      )}
...
interface IgnoredItemProps {
  item: IgnoreItem;
  revalidateList: () => void;
}

const IgnoredItem = ({ item, revalidateList }: IgnoredItemProps) => {
  ...
  const url = `/api/v1/tv/${item.tmdbId}`;
    ...
    <div className="relative flex w-full flex-col justify-between overflow-hidden rounded-xl bg-gray-800 py-4 text-gray-400 shadow-md ring-1 ring-gray-700 xl:h-28 xl:flex-row">
        ...
        <div className="z-10 mt-4 ml-4 flex w-full flex-col justify-center overflow-hidden pr-4 text-sm sm:ml-2 sm:mt-0 xl:flex-1 xl:pr-0">
          <div className="card-field">
            ...
            <Badge badgeType="danger">
              {intl.formatMessage(globalMessages.ignored)}
            </Badge>
          </div>
          ...
          {item.createdAt && (
            <div className="card-field">
              ...
              <span className="flex truncate text-sm text-gray-300">
                {intl.formatMessage(messages.ignoredby, {
                  date: (
      ...
      <div className="z-10 mt-4 flex w-full flex-col justify-center space-y-2 pl-4 pr-4 xl:mt-0 xl:w-96 xl:items-end xl:pl-0">
        {hasPermission(Permission.ADMIN) && (
          ...
          >
            <TrashIcon />
            <span>{intl.formatMessage(globalMessages.removefromIgnore)}</span>
          </ConfirmButton>
        )}
      </div>
    </div>
